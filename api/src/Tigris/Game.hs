...
playInteraction :: Monad m => (Dynasty -> Interactions m) -> Interaction -> [Dynasty] -> Game -> m (Either Winners PlayingState)
playInteraction interactions' interaction (currentPlayer:subsequentPlayers) game = case interaction of
  Turn turnNumber -> flip applyAction game <$> getAction playerInteractions where
    applyAction Pass = endTurn
    applyAction (ReplaceTiles discards) = continue <=< undefined
--    applyAction (ReplaceTiles discards) = continue =<< maybe (Left $ determineWinners game) Right ((playerAndBag . state (at currentPlayer . traverse . hand)) (dealUpToSix . (<>) discards) game)
    endTurn = uncurry (maybe (Right . nextPlayer) (const . Left)) . runState (whenM (fmap Just determineWinners) . isFinished =<< orContinueWith (playersAndBag . onFirst (traverse . hand) %%= uncurry refreshHand) =<< finishedDueToTreasures)
    continue = case turnNumber of
      FirstAction -> Right . PlayingState (Turn SecondAction) (currentPlayer:subsequentPlayers)
      SecondAction -> endTurn
    orContinueWith f = bool f (return $ IsFinished True) . isFinished
    refreshHand :: Hand -> [Tile] -> (IsFinished, (Hand, [Tile]))
    refreshHand hand' = deal (6 - Bag.size hand') hand' where
      deal :: Int -> Hand -> [Tile] -> (IsFinished, (Hand, [Tile]))
      deal n h ts = bool (IsFinished False, (h, ts)) (maybe (IsFinished True, (h, [])) (uncurry $ deal (n - 1) . (<>h) . Bag.one) $ List.uncons ts) (n > 0)
    finishedDueToTreasures :: State Game IsFinished
    finishedDueToTreasures = undefined
    nextPlayer = PlayingState (Turn FirstAction) subsequentPlayers 
    playerInteractions = interactions' currentPlayer
playInteraction _ _ [] _ = undefined
...
onFirst :: Functor f => ((a -> Compose f ((,) c) b) -> s -> Compose f ((,) c) t) -> ((a, c) -> f (b, c)) -> (s, c) -> f (t, c)
onFirst l f (s, c) = fmap swap $ getCompose $ l g s where
  g a = Compose $ fmap swap $ f (a, c)
